...
| **SIRL-1222** | Separador `;` em `CRRCORP.dat` / `CRRADAPT.dat` | 🟡 **P1 validado no DEV2** (554045 linhas, 662 `;` por linha nas posições da Notice) + **os outros 6 pavés gerados e conferidos estaticamente** (F2, F1, P9, P2, M1, C1 — 7 correções de largura ao todo, cabeçalho e rodapé já vinham com `;`); falta a corrida no DEV2 e o Adapté — ver [docs/SIRL-1222-ALINHAMENTO.md](docs/SIRL-1222-ALINHAMENTO.md) |
...
| `mapa_paves.py` | SIRL-1222: acha os blocos dos outros seis paves pela tabela do FROM e mede a largura de cada token |
| `casa_paves.py` | SIRL-1222: alinha, por programacao dinamica, os campos da Notice com os tokens de cada pave, e lista as divergencias |
